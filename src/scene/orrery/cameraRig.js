...
export function createCameraRig(camera, controls, homePosition) {
  ...
  const startFlight = (target, endDistance, direction, duration) => {
    flight = {
      target,
      elapsed: 0,
      duration,
  ...
  return {
    ...
    flyTo(nextSubject, { duration = 1.6, preferredDirection = null } = {}) {
      subject = nextSubject;
      ...
      startFlight(nextSubject, Math.max(nextSubject.getRadius() * 4, MIN_VIEW_DISTANCE), direction, duration);
    },
    ...
    update(dtMs) {
      if (flight) {
        ...
        const distance =
          Math.exp(Math.log(flight.startDistance) * (1 - p) + Math.log(flight.endDistance) * p) + flight.arc * Math.sin(Math.PI * p);
